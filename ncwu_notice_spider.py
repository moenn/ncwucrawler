...
def get_data_from_pageurl(pageurl):
    ...
    pattern = re.compile(
        ...
        r' target="_blank"><span> (.+) </span> </a><i>(\d{4}-\d{2}-\d{2})</i></li>'
    )
    result = re.finditer(pattern, resp.text)

    data = {'http://www5.ncwu.edu.cn/contents/{}.html'.format(n.group(2)): {'department': n.group(1), 'title': n.group(
        3), 'href': 'http://www5.ncwu.edu.cn/contents/{}.html'.format(n.group(2)), 'create_time': n.group(4)} for n in result}
...
def download(data, startpage_num, root_dir):
    ...
    for key, value in data.items():
        ...
        if count % 12 == 0:
            page_dir_path = os.path.join(
                root_dir, 'notice_5_' + str(startpage_num))
